...
public class TileEntityRecycler extends TileEntity implements ISidedInventory{
	...
	*/
	...
	{
    	return this.isInvNameLocalized() ? this.field_94130_e : "container.recycler";
	}
	...
	public void updateEntity(){
    	...
    	if (!this.worldObj.isRemote){
        	...
        	{
            	flag1 = true;
            	//BlockElectricFurnace.updateFurnaceBlockState(this.power > 0, this.worldObj, this.xCoord, this.yCoord, this.zCoord);
        	}	
    	}
...
//    	}
		if(this.slots[0] == null) {
			return false;
		} else if(this.slots[0].itemID == Sustain.blockTrash.blockID){
			//System.out.println("TRASH IN PLACE!");
			return true;
		} else {
			return false;
		}
	...
	public void smeltItem(){
    	if(this.canSmelt()){
    		...
    		if(n >= 0 && n < 30) {
    			itemstack = new ItemStack(Block.oreIron);
    		} else if(n >= 30 && n < 60) {
    			itemstack = new ItemStack(Sustain.blockAluminumOre);
    		} else if(n >= 60 && n < 90) {
    			itemstack = new ItemStack(Item.coal);
    		} else if(n >= 90 && n < 100) {
    			itemstack = new ItemStack(Sustain.blockTrash);
    		} else {
    			itemstack = new ItemStack(Block.cactus);
    		}
    		...
    		while(!isDone) {
    			if(this.slots[slotCounter] == null) {
    				this.slots[slotCounter] = itemstack.copy();
    				this.slots[slotCounter].stackSize*=2;
    				...
    				isDone = true;
    			} else if(this.slots[slotCounter].stackSize == 64) {		
    				slotCounter++;
    				if(slotCounter == 11) {
    					System.out.println("Out of space!");
    					isDone = true;
    				}
    			} else if(this.slots[slotCounter].isItemEqual(itemstack)) {
    				slots[slotCounter].stackSize += (itemstack.stackSize*2);
    				--this.slots[0].stackSize;
	...
	public static int getItemPower(ItemStack par0ItemStack){
    	if (par0ItemStack == null){
    		...
    		if(i == Sustain.itemBattery.itemID) return 1;
    		if(i == Sustain.itemBatteryPack.itemID) return 1;
